...
import pytesseract
import telepot
bot=telepot.Bot("7886064129:AAHWe-JBZtqElm0tyAOU6azE3C3F5DKKHLQ")
...
def detect():
    ...
    db.execute("INSERT INTO :tablename ('text', 'nature', 'image') VALUES (:post_text, :score, :post_img)", tablename=userInfo['username'], post_text=post_text, score=str(ans), post_img=path)
    if (ans < 0.4):
        score = (0.4 - ans)
    ...
    if request.method == "GET":
        ...
        if get_posts:
            ...
            if ((userInfo['score']/userInfo['total'])*10) < 5:
                ...
                print('blocked')
                bot.sendMessage("5486829784",str("your accoint is blocked"))
##                abort(403, "ip blocked due to score is les than 5")
        ...
        fromlang = request.form.get("lang")
